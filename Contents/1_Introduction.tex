...
Mục tiêu là thiết kế thiết bị đọc ghi và giải mã màn hình cây xăng đáp ứng những nhu cầu sau:
 ...
 Điều khiển các relay bật tắt từ xa thông qua giao diện người dùng.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.4\linewidth]{Figures/ZCheng_gas-pump-screen.jpg}
    \hspace{0.1cm}
    \includegraphics[width=0.4\linewidth]{Figures/ZCheng_screen-control-ic.jpg}
    
    \caption{Màn hình và mạch phần cứng màn hình cây xăng hãng ZCheng}
    \label{fig:hinh1.1}
\end{figure}
\FloatBarrier
...
Từ đó, chế tạo mạch thu gói tin theo giao thức SPI và thiết kế phần mềm giải mã. Hình \ref{fig:logic_analyzer_data_sample} là một mẫu tín hiệu màn hình thu được vào thời điểm màn hình hiển thị:

\begin{itemize}
   \item Sale: \texttt{4.88}
   \item Litre: \texttt{16.37}
   \item Price: \texttt{79.89}
\end{itemize}

\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\linewidth]{Figures/logic_analyzer_data_sample.png}
   
    \caption{Một mẫu tín hiệu màn hình thu được sử dụng Logic Analyzer}
    \label{fig:logic_analyzer_data_sample}
\end{figure}
